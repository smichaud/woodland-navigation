...
inline Eigen::Matrix<real,3,3> getMatrixFromEuler(real roll, real pitch, real yaw) {
  ...
  ret(0,0) = cosAlpha*cosBeta;
  ret(0,1) = cosAlpha*sinBeta*sinGamma - sinAlpha*cosGamma;
  ret(0,2) = cosAlpha*sinBeta*cosGamma + sinAlpha*sinGamma;
  ret(1,0) = sinAlpha*cosBeta;
  ret(1,1) = sinAlpha*sinBeta*sinGamma + cosAlpha*cosGamma;
  ret(1,2) = sinAlpha*sinBeta*cosGamma - cosAlpha*sinGamma;
  ret(2,0) = -sinBeta;
  ret(2,1) = cosBeta*sinGamma;
  ret(2,2) = cosBeta*cosGamma;
  return ret;
}
...
inline Eigen::Matrix<real,4,4> getMatrixFromTranslationAndEuler(real x, real y, real z, real roll, real pitch, real yaw) {
  ...
  ret(2,2) = cosBeta*cosGamma;
  ret(2,3) = z;
  ret(3,0)=ret(3,1)=ret(3,2)=0.0f, ret(3,3)=1.0f;
  return ret;
}
...
inline void getEulerAngles(Eigen::Transform<real1,3,Eigen::Affine>& t, real2& roll, real2& pitch, real2& yaw)
{
...
template <typename real1, typename real2>
inline void getTranslationAndEulerAngles(Eigen::Transform<real1,3,Eigen::Affine>& t, real2& x, real2& y, real2& z, real2& roll, real2& pitch, real2& yaw)
{
  x = t(0,3);
  y = t(1,3);
  z = t(2,3);
...
{
  if (normal1==normal2) {
    transformation = Eigen::Matrix<real, 4, 4>::Identity();
  }
  else {
    ...
    transformation1 = Eigen::AngleAxis<real>(rotation, Eigen::Vector3f(1.0f, 0.0f, 0.0f))*transformation1;
