...
struct SparsePointCloud {
  ...
  void getIndicesFromCoordinates(float x, float y, float z, int& idxX, int& idxY, int& idxZ) {
    idxX=lrintf(x/cellSize), idxY=lrintf(y/cellSize), idxZ=lrintf(z/cellSize);
  }
  void getCoordinatesFromIndices(int idxX, int idxY, int idxZ, float& x, float& y, float& z) {
    x = float(idxX)*cellSize,  y = float(idxY)*cellSize,  z = float(idxZ)*cellSize;
  }
  bool checkExistence(float x, float y, float z) {
    IteratorX itX = points.find(lrintf(x/cellSize));
    if (itX==points.end())  return false;
    IteratorY itY = itX->second.find(lrintf(y/cellSize));
    if (itY==itX->second.end())  return false;
    IteratorZ itZ = itY->second.find(lrintf(z/cellSize));
